...
def SimulatedAnnealingProcess(rng, origin_context: GridMovingContext, bound1, bound2, bound3, alpha, currentstep,agent_scores):
...
class SADualObjectScheduling(IStrategy):
    ...
    def get(self, model: IModel, Setting, pred, agent_scores) -> np.ndarray:
        ...
        if self.moving_context is None:
          ...
          self.moving_context, sq_list_total = SimulatedAnnealingProcess(self.rng, self.moving_context, Setting.bound1, Setting.bound2, Setting.bound3, self.alpha, Setting.current_step, agent_scores)
