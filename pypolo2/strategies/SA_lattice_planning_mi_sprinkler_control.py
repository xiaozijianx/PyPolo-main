        ...
        if delta_spraytime <= 0 or enough_info[rand_agent] == True:
          if MI_after < object_mi[rand_agent]:
            ...
            if delta_sprayeffect >= 0:
              ...
              accept_prob = np.exp(delta_sprayeffect / (curr_k * Temp * 125))
              if(random.random() < accept_prob):
                curr_context = new_mc_context
...
def SimulatedAnnealingFixed(origin_context: GridMovingContext, bound, alpha, beta):
  ...
  for i in range(context.GetAgentNumber()):
    if sprayeffect_after_everyvehicle[i] > 120:
      alpha_now = 0.7
      enough_info[i] = True
    elif sprayeffect_after_everyvehicle[i] > 90:
      alpha_now = 0.95
      enough_info[i] = True
    else:
      alpha_now = alpha
  ...
  # 根据车辆的观测判断是否采集了足够的信息
  if origin_context.GetCurrentInfo() > 20:
    for i in range(context.GetAgentNumber()):
      enough_info[i] = True
  print('enough_info')
  print(enough_info)
...
class SALatticePlanningMISprinklerControl(IStrategy):
    ...
    def get(self, model: IModel, Setting, pred) -> np.ndarray:
        ...
        for num in range(0,sche_step,3):
          for i in range (self.task_extent[0]-2,self.task_extent[1]+2,2):
              for j in range (self.task_extent[2]-2,self.task_extent[3]+2,2):
