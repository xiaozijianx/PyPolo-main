...
def SimulatedAnnealingFixed(origin_context: GridMovingContext, bound, alpha, beta):
  ...
  if (current_info-last_info)/last_info < 0.1:
    for i in range(context.GetAgentNumber()):
      enough_info[i] = True
  ...
  single_playout = np.ceil(origin_context.GetAgentNumber()*20*np.exp(1/(origin_context.Setting.current_step//2+2)) - origin_context.GetAgentNumber()*2*20//4)
...
#定义SA算法包装
class SALatticePlanningMISprinklerControl_mimethod3(IStrategy):
    ...
    def get(self, model: IModel, Setting, pred) -> np.ndarray:
        ...
        allpoint_list = []
        a = ((np.ceil((self.task_extent[1]-self.task_extent[0])/3)*3)-(self.task_extent[1]-self.task_extent[0]-1))/2
        for num in range(0,sche_step,3):
          for i in np.arange (self.task_extent[0]-a,self.task_extent[1]+a,3):
              for j in np.arange (self.task_extent[2]-a,self.task_extent[3]+a,3):
