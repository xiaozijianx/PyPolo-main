...
class TraversalLatticePlanningMISprinklerControl(IStrategy):
    """Myopic informative planning based on Mutual informaiton on latttice map.
    遍历算法，使用局部搜索，在计算信息量时，将所选择的点集按经过的时间顺序，放在合适的时间面上，计算整体的信息量
    """
    ...
    def greedy_search_multi_step(self, length, alpha, sprayeffect_all, model):
        ...
        for id, vehicle in self.vehicle_team.items():
            ...
            while nodes:
                ...
                if current_length == 0:
                    new_x = np.array([current_node[0],current_node[1],model.time_stamp]).reshape(1,-1)
                    ...
                    # print("points", select_points, processed_points2,"mi", mi,"  effect", spray_effect)
    ...
    def get(self, model: IModel, alpha = 1, step_number = 4) -> np.ndarray:
        ...
        result = self.greedy_search_multi_step(step_number, alpha, sprayeffect_all, model)
