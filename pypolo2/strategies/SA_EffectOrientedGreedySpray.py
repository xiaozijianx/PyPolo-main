...
from ..gridcontext.GridMovingContextGreedy import GridMovingContext as GridMovingContext
...
def SimulatedAnnealing(rng, origin_mc_context: GridMovingContext, *,enough_info = None, n_playout=10000, initial_temp=1, k=0.95, bound=100, min_temp= 0.001, 
  ...
  while(curr_turns < bound):
    ...
    sq_list.append((sprayeffect_after, 0, 0))
...
def SimulatedAnnealingFixed(rng, origin_context: GridMovingContext, bound, alpha):
  ...
  # Spray_Temp = np.max((2500 - origin_context.Setting.current_step * 100,1500))
  Spray_Temp = 1
...
#定义SA算法包装
class SAEffectOrientedGreedySpray(IStrategy):
