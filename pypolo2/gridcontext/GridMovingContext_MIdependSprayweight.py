...
# 这里，计算洒水收益时同时考虑时间衰减和不确定性
class GridMovingContext_MIDependSprayweight():
          ...
          for a in range(3):
            for b in range(3):
              ...
              if r >= 0 and r < self.map_shape[0] and c >= 0 and c < self.map_shape[1]:
                if a == 1 and b == 1:
                  spray_effect = spray_effect + (0.85+0.1*self.MIforeverypoint[r,c])**j*calculate_effect(pollution_distribute[r,c])
                  ...
                  spray_effect = spray_effect + 0.5*(0.85+0.1*self.MIforeverypoint[r,c])**j*calculate_effect(pollution_distribute[r,c])
        ...
        if self.policy_matrix[i, j, 2] == 1:
          ...
          for a in range(3):
            for b in range(3):
              ...
              if r >= 0 and r < self.map_shape[0] and c >= 0 and c < self.map_shape[1]:
                if a == 1 and b == 1:
                  spray_effect[i] = spray_effect[i] + (0.85+0.1*self.MIforeverypoint[r,c])**j*calculate_effect(pollution_distribute[r,c])
                  spray_effect0 = spray_effect0 + (0.85+0.1*self.MIforeverypoint[r,c])**j*calculate_effect(pollution_distribute[r,c])
                  pollution_distribute[r,c] = pollution_distribute[r,c] - calculate_effect(pollution_distribute[r,c])
                else:
                  spray_effect[i] = spray_effect[i] + 0.5*(0.85+0.1*self.MIforeverypoint[r,c])**j*calculate_effect(pollution_distribute[r,c])
                  spray_effect0 = spray_effect0 + 0.5*(0.85+0.1*self.MIforeverypoint[r,c])**j *calculate_effect(pollution_distribute[r,c])
                  pollution_distribute[r,c] = pollution_distribute[r,c] - 0.5*calculate_effect(pollution_distribute[r,c])
