...
class Evaluator:
    ...
    def eval_prediction(
        ...
        if x_train.shape[1] != self.eval_inputs.shape[1]:
            model_input = np.zeros((self.eval_inputs.shape[0],x_train.shape[1]))
            model_input[:,0:self.eval_inputs.shape[1]] = self.eval_inputs
            model_input[:,self.eval_inputs.shape[1]:x_train.shape[1]] = model.time_stamp
        else:
            model_input = self.eval_inputs
            
        mean, std = model(model_input)
